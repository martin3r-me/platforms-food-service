<x-foodservice-page
    :title="$cluster->name"
    icon="heroicon-o-rectangle-group"
    :breadcrumbs="[
        ['label' => 'Artikel-Cluster', 'href' => route('foodservice.article-clusters.index')],
    ]"
>
    <x-slot name="actions">
        <x-ui-button variant="secondary-outline" :href="route('foodservice.article-clusters.index')" wire:navigate>
            @svg('heroicon-o-arrow-left','w-4 h-4')
            Übersicht
        </x-ui-button>
        @if($this->isDirty)
            <x-ui-button variant="primary" wire:click="save">
                @svg('heroicon-o-check','w-4 h-4')
                Speichern
            </x-ui-button>
        @endif
    </x-slot>

    <x-slot name="sidebar">
        <div class="space-y-6">
            <div class="p-4 rounded-xl border border-[var(--ui-border)]/50 bg-[var(--ui-muted-5)]">
                <h4 class="text-sm font-semibold text-[var(--ui-secondary)] mb-3">Status</h4>
                <x-ui-badge :variant="$cluster->is_active ? 'success' : 'secondary'" size="sm">
                    {{ $cluster->is_active ? 'Aktiv' : 'Inaktiv' }}
                </x-ui-badge>
            </div>

            <x-ui-input-checkbox
                model="cluster.is_active"
                checked-label="Aktiv"
                unchecked-label="Inaktiv"
                size="md"
                block="true"
            />

            <x-ui-confirm-button 
                action="deleteCluster"
                text="Löschen" 
                confirmText="Wirklich löschen?" 
                variant="danger-outline"
                :icon="@svg('heroicon-o-trash', 'w-4 h-4')->toHtml()"
            />
        </div>
    </x-slot>

    <x-slot name="activity">
        <livewire:activity-log.index
            :model="$cluster"
            :key="get_class($cluster) . '_' . $cluster->id"
        />
    </x-slot>

    <div class="space-y-6">
        <x-ui-panel title="Meta">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <x-ui-input-text 
                    name="cluster.name"
                    label="Name"
                    wire:model.live="cluster.name"
                    required
                    :errorKey="'cluster.name'"
                />
                <x-ui-input-textarea 
                    name="cluster.description"
                    label="Beschreibung"
                    wire:model.live="cluster.description"
                    rows="4"
                    :errorKey="'cluster.description'"
                    class="md:col-span-2"
                />
            </div>
        </x-ui-panel>
    </div>
</x-foodservice-page>

