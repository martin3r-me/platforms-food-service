...
>
    <x-slot name="sidebar">
        @php
            $totalAttributes = $items->count();
            $activeAttributes = $items->where('is_active', true)->count();
            $strictAttributes = $items->where('is_strict', true)->count();
        @endphp
        <div class="space-y-6">
            <div class="space-y-3">
            ...
            <div>
                ...
                <div class="space-y-2">
                    <div class="p-3 rounded-lg border border-[var(--ui-border)]/40 bg-[var(--ui-muted-5)]">
                        ...
                        <p class="text-lg font-semibold text-[var(--ui-secondary)]">{{ $totalAttributes }}</p>
                    </div>
                    <div class="p-3 rounded-lg border border-[var(--ui-border)]/40 bg-[var(--ui-muted-5)]">
                        <p class="text-xs text-[var(--ui-muted)]">Aktiv</p>
                        <p class="text-lg font-semibold text-[var(--ui-secondary)]">{{ $activeAttributes }}</p>
                    </div>
                    <div class="p-3 rounded-lg border border-[var(--ui-border)]/40 bg-[var(--ui-muted-5)]">
                        <p class="text-xs text-[var(--ui-muted)]">Streng (Hard)</p>
                        <p class="text-lg font-semibold text-[var(--ui-secondary)]">{{ $strictAttributes }}</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <x-slot name="activity">
        ...
        <div class="space-y-3">
            ...
            <div class="space-y-2">
                @forelse($recentAttributes as $recent)
                    <a
                        href="{{ route('foodservice.attributes.show', ['attribute' => $recent]) }}"
                        wire:navigate
