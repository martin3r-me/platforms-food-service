<x-foodservice-page
    ...
    description="Verwalte alle Allergene samt Hierarchien"
>
    <x-slot name="sidebar">
        @php
            $totalAllergens = $items->count();
            $activeAllergens = $items->where('is_active', true)->count();
            $strictAllergens = $items->where('is_strict', true)->count();
        @endphp
        <div class="space-y-6">
            <div class="space-y-3">
            ...
            <div>
                ...
                <div class="space-y-2">
                    <div class="p-3 rounded-lg border border-[var(--ui-border)]/40 bg-[var(--ui-muted-5)]">
                        ...
                        <p class="text-lg font-semibold text-[var(--ui-secondary)]">{{ $totalAllergens }}</p>
                    </div>
                    <div class="p-3 rounded-lg border border-[var(--ui-border)]/40 bg-[var(--ui-muted-5)]">
                        <p class="text-xs text-[var(--ui-muted)]">Aktiv</p>
                        <p class="text-lg font-semibold text-[var(--ui-secondary)]">{{ $activeAllergens }}</p>
                    </div>
                    <div class="p-3 rounded-lg border border-[var(--ui-border)]/40 bg-[var(--ui-muted-5)]">
                        <p class="text-xs text-[var(--ui-muted)]">Streng (Hard)</p>
                        <p class="text-lg font-semibold text-[var(--ui-secondary)]">{{ $strictAllergens }}</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <x-slot name="activity">
        ...
        <div class="space-y-3">
            ...
            <div class="space-y-2">
                @forelse($recentAllergens as $recent)
                    <a
                        href="{{ route('foodservice.allergens.show', ['allergen' => $recent]) }}"
                        wire:navigate
    ...
    <div class="flex items-center justify-between mb-6">
        <p class="text-sm text-[var(--ui-muted)]">Übersicht aller Allergene</p>
    ...
    @else
        <x-ui-empty-state
            icon="heroicon-o-exclamation-triangle"
            title="Noch keine Allergene"
            description="Lege das erste Allergen an, um Artikel korrekt zu kennzeichnen."
        >
            <x-ui-button variant="primary" wire:click="openCreateModal">
                Allergen erstellen
            </x-ui-button>
    ...
    <x-ui-modal wire:model="modalShow" size="md">
        <x-slot name="header">Allergen erstellen</x-slot>
