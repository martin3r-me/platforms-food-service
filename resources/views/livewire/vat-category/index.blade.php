...
>
    <x-slot name="sidebar">
        @php
            $totalVatCategories = $items->count();
            $activeVatCategories = $items->where('is_active', true)->count();
            $withRates = $items->filter(fn($item) => $item->rates()->exists())->count();
        @endphp
        <div class="space-y-6">
            <div class="space-y-3">
            ...
            <div>
                ...
                <div class="space-y-2">
                    <div class="p-3 rounded-lg border border-[var(--ui-border)]/40 bg-[var(--ui-muted-5)]">
                        ...
                        <p class="text-lg font-semibold text-[var(--ui-secondary)]">{{ $totalVatCategories }}</p>
                    </div>
                    <div class="p-3 rounded-lg border border-[var(--ui-border)]/40 bg-[var(--ui-muted-5)]">
                        <p class="text-xs text-[var(--ui-muted)]">Aktiv</p>
                        <p class="text-lg font-semibold text-[var(--ui-secondary)]">{{ $activeVatCategories }}</p>
                    </div>
                    <div class="p-3 rounded-lg border border-[var(--ui-border)]/40 bg-[var(--ui-muted-5)]">
                        <p class="text-xs text-[var(--ui-muted)]">Mit gültigem Satz</p>
                        <p class="text-lg font-semibold text-[var(--ui-secondary)]">{{ $withRates }}</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <x-slot name="activity">
        ...
        <div class="space-y-3">
            ...
            <div class="space-y-2">
                @forelse($recentVatCategories as $recent)
                    <a
                        href="{{ route('foodservice.vat-categories.show', ['category' => $recent]) }}"
                        wire:navigate
