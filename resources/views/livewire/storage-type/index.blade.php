<x-foodservice-page
    ...
    description="Strukturiere Lagerarten und Hierarchien"
>
    <x-slot name="sidebar">
        @php
            $totalStorageTypes = $items->count();
            $activeStorageTypes = $items->where('is_active', true)->count();
            $topLevelStorageTypes = $items->whereNull('parent_id')->count();
        @endphp
        <div class="space-y-6">
            <div class="space-y-3">
            ...
            <div>
                ...
                <div class="space-y-2">
                    <div class="p-3 rounded-lg border border-[var(--ui-border)]/40 bg-[var(--ui-muted-5)]">
                        ...
                        <p class="text-lg font-semibold text-[var(--ui-secondary)]">{{ $totalStorageTypes }}</p>
                    </div>
                    <div class="p-3 rounded-lg border border-[var(--ui-border)]/40 bg-[var(--ui-muted-5)]">
                        <p class="text-xs text-[var(--ui-muted)]">Aktiv</p>
                        <p class="text-lg font-semibold text-[var(--ui-secondary)]">{{ $activeStorageTypes }}</p>
                    </div>
                    <div class="p-3 rounded-lg border border-[var(--ui-border)]/40 bg-[var(--ui-muted-5)]">
                        <p class="text-xs text-[var(--ui-muted)]">Top-Level</p>
                        <p class="text-lg font-semibold text-[var(--ui-secondary)]">{{ $topLevelStorageTypes }}</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <x-slot name="activity">
        ...
        <div class="space-y-3">
            ...
            <div class="space-y-2">
                @forelse($recentStorageTypes as $recent)
                    <a
                        href="{{ route('foodservice.storage-types.show', ['storageType' => $recent]) }}"
                        wire:navigate
    ...
    <div class="flex items-center justify-between mb-6">
        <p class="text-sm text-[var(--ui-muted)]">Alle Lagerarten inkl. Hierarchie</p>
        ...
        <x-ui-table compact="true">
            ...
            <x-ui-table-body>
                @foreach($items as $item)
                    <x-ui-table-row compact="true" clickable="true" :href="route('foodservice.storage-types.show', ['storageType' => $item])" wire:navigate>
                        ...
                        <x-ui-table-cell compact="true" align="right">
                            <x-ui-button size="sm" variant="secondary" :href="route('foodservice.storage-types.show', ['storageType' => $item])" wire:navigate>
                                Open
                            </x-ui-button>
                        </x-ui-table-cell>
                    </x-ui-table-row>
    ...
    @else
        <div class="text-center py-8 text-sm text-muted">No storage types yet</div>
    @endif

    <x-ui-modal wire:model="modalShow" size="md">
        <x-slot name="header">Create Storage Type</x-slot>
        ...
        <form wire:submit.prevent="createItem" class="space-y-4">
            ...
            <x-ui-input-select
                name="parent_id"
                label="Parent"
                :options="\Platform\FoodService\Models\FsStorageType::orderBy('name')->get()"
                optionValue="id"
                optionLabel="name"
                :nullable="true"
                nullLabel="– None –"
