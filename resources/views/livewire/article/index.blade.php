<x-foodservice-page
    ...
    description="Alle Einkaufsartikel mit relevanten Stammdaten"
>
    <x-slot name="sidebar">
        @php
            $articles = $this->articles;
            $totalArticles = $articles->count();
            $activeArticles = $articles->where('is_active', true)->count();
            $withBrand = $articles->whereNotNull('brand_id')->count();
        @endphp
        <div class="space-y-6">
            <div class="space-y-3">
            ...
            <div>
                ...
                <div class="space-y-2">
                    <div class="p-3 rounded-lg border border-[var(--ui-border)]/40 bg-[var(--ui-muted-5)]">
                        ...
                        <p class="text-lg font-semibold text-[var(--ui-secondary)]">{{ $totalArticles }}</p>
                    </div>
                    <div class="p-3 rounded-lg border border-[var(--ui-border)]/40 bg-[var(--ui-muted-5)]">
                        <p class="text-xs text-[var(--ui-muted)]">Aktiv</p>
                        <p class="text-lg font-semibold text-[var(--ui-secondary)]">{{ $activeArticles }}</p>
                    </div>
                    <div class="p-3 rounded-lg border border-[var(--ui-border)]/40 bg-[var(--ui-muted-5)]">
                        <p class="text-xs text-[var(--ui-muted)]">Mit Marke</p>
                        <p class="text-lg font-semibold text-[var(--ui-secondary)]">{{ $withBrand }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="space-y-3">
            ...
            <div class="space-y-2">
                @forelse($recentArticles as $recent)
                    <a
                        href="{{ route('foodservice.articles.show', ['article' => $recent]) }}"
                        wire:navigate
    ...
    <div class="flex items-center justify-between mb-6">
        <p class="text-sm text-[var(--ui-muted)]">Übersicht aller Einkaufsartikel</p>
